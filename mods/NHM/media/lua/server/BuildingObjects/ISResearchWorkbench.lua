...
ISResearchWorkbench = ISBuildingObject:derive("ISResearchWorkbench");

--************************************************************************--
--** ISResearchWorkbench:new
--**
--************************************************************************--
function ISResearchWorkbench:create(x, y, z, north, sprite)
    local cell = getWorld():getCell();
    self.sq = cell:getGridSquare(x, y, z);
    self.javaObject = IsoThumpable.new(cell, self.sq, sprite, north, self);
    self.javaObject:setName("Research Workbench");
...
function ISResearchWorkbench:removeFromGround(square)
    for i = 0, square:getSpecialObjects():size() do
        local thump = square:getSpecialObjects():get(i);
        if instanceof(thump, "Research Workbench") then
...
function ISResearchWorkbench:new(name, character, sprite, northSprite)
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o:init();
...
function ISResearchWorkbench:isValid(square)
...
function ISResearchWorkbench:render(x, y, z, square)
