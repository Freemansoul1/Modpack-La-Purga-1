...
ISElectricWorkbench = ISBuildingObject:derive("ISElectricWorkbench");

--************************************************************************--
--** ISElectricWorkbench:new
--**
--************************************************************************--
function ISElectricWorkbench:create(x, y, z, north, sprite)
    local cell = getWorld():getCell();
    self.sq = cell:getGridSquare(x, y, z);
    self.javaObject = IsoThumpable.new(cell, self.sq, sprite, north, self);
    self.javaObject:setName("Electric Workbench");
...
function ISElectricWorkbench:removeFromGround(square)
    for i = 0, square:getSpecialObjects():size() do
        local thump = square:getSpecialObjects():get(i);
        if instanceof(thump, "Electric Workbench") then
...
function ISElectricWorkbench:new(name, character, sprite, northSprite)
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o:init();
...
function ISElectricWorkbench:isValid(square)
...
function ISElectricWorkbench:render(x, y, z, square)
