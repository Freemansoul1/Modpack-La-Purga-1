...
ISLeatherworkingWorkbench = ISBuildingObject:derive("ISLeatherworkingWorkbench");

--************************************************************************--
--** ISLeatherworking:new
--**
--************************************************************************--
function ISLeatherworkingWorkbench:create(x, y, z, north, sprite)
    local cell = getWorld():getCell();
    self.sq = cell:getGridSquare(x, y, z);
    self.javaObject = IsoThumpable.new(cell, self.sq, sprite, north, self);
    self.javaObject:setName("Leatherworking Workbench");
...
function ISLeatherworkingWorkbench:removeFromGround(square)
    for i = 0, square:getSpecialObjects():size() do
        local thump = square:getSpecialObjects():get(i);
        if instanceof(thump, "Leatherworking Workbench") then
...
function ISLeatherworkingWorkbench:new(name, character, sprite, northSprite)
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o:init();
...
function ISLeatherworkingWorkbench:isValid(square)
...
function ISLeatherworkingWorkbench:render(x, y, z, square)
