...
ISChemistryWorkbench = ISBuildingObject:derive("ISChemistryWorkbench");

--************************************************************************--
--** ISChemistryWorkbench:new
--**
--************************************************************************--
function ISChemistryWorkbench:create(x, y, z, north, sprite)
    local cell = getWorld():getCell();
    self.sq = cell:getGridSquare(x, y, z);
    self.javaObject = IsoThumpable.new(cell, self.sq, sprite, north, self);
    self.javaObject:setName("Chemistry Workbench");
...
function ISChemistryWorkbench:removeFromGround(square)
    for i = 0, square:getSpecialObjects():size() do
        local thump = square:getSpecialObjects():get(i);
        if instanceof(thump, "Chemistry Workbench") then
...
function ISChemistryWorkbench:new(name, character, sprite, northSprite)
    local o = {};
    setmetatable(o, self);
    self.__index = self;
    o:init();
...
function ISChemistryWorkbench:isValid(square)
...
function ISChemistryWorkbench:render(x, y, z, square)
