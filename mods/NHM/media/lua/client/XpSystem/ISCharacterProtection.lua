...
function ISCharacterProtection:render()
	...
	for i=0, BodyPartType.ToIndex(BodyPartType.MAX) do
		...
		if self.bparts[string] then
            ...
            local bulletDefence = luautils.round(self.char:getBodyPartClothingDefense(i, false, true));
			biteDefense = math.floor(biteDefense);
			scratchDefense = math.floor(scratchDefense);
            bulletDefence = math.floor(bulletDefence);
			-- COMMENT MERGE:
            ...
            self.bodyPartPanel:setValue( BodyPartType.FromIndex(i), (biteDefense + scratchDefense + bulletDefence));
            ...
            r, g, b = self.bodyPartPanel:getRgbForValue( bulletDefence );
            self:drawText(bulletDefence .. "%", bulletX, yText, r, g, b, 1, UIFont.Small);
	...
	local width = math.max(self.width, scratchX + scratchWidth + bulletWidth + 30);
