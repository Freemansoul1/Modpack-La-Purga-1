    ...
    if isWeapon and instanceof(isWeapon, "HandWeapon") and hasScrewdriver then
        ...
        if weaponParts and not weaponParts:isEmpty() then
            ...
            for i=0, weaponParts:size() - 1 do
                ...
                if addOption then
                    doIt = true;
                    local partOption = subMenuUp:addOption(weaponParts:get(i):getName(), isWeapon, ISInventoryPaneContextMenu.onUpgradeWeapon, part, getSpecificPlayer(player));
                    local weaponScript = isWeapon:getScriptItem();
                    if ((part:getPartType() == "Clip" or part:getPartType() == "Canon" or part:getPartType() == "RecoilPad") and not isWeapon:getSling() and TableHasValue(AutomaticRiflesTable, weaponScript:getName())) then
                        local tooltip = ISInventoryPaneContextMenu.addToolTip();
                        partOption.notAvailable = true;
                        tooltip:setName(part:getName());
                        tooltip.description = tooltip.description .. getText("Tooltip_upgrade_Handguard_Warning");
                        partOption.toolTip = tooltip;
                    end
                    if ((part:getPartType() == "Sling" or part:getPartType() == "Scope" or part:getPartType() == "RecoilPad" or part:getPartType() == "Clip") and not isWeapon:getStock() and TableHasValue(SingleShotRiflesTable, weaponScript:getName())) then
                        local tooltip = ISInventoryPaneContextMenu.addToolTip();
                        partOption.notAvailable = true;
                        tooltip:setName(part:getName());
                        tooltip.description = tooltip.description .. getText("Tooltip_upgrade_Forearm_Warning");
                        partOption.toolTip = tooltip;
                    end
                    addOption = false;
        ...
        if hasScrewdriver and (isWeapon:getScope() or isWeapon:getClip() or isWeapon:getSling() or isWeapon:getStock() or isWeapon:getCanon() or isWeapon:getRecoilpad()) then
            ...
            if isWeapon:getStock() then
                local option = subMenuRemove:addOption(isWeapon:getStock():getName(), isWeapon, ISInventoryPaneContextMenu.onRemoveUpgradeWeapon, isWeapon:getStock(), getSpecificPlayer(player));
                local weaponScript = isWeapon:getScriptItem();
                if (isWeapon:getClip() or isWeapon:getScope() or isWeapon:getRecoilpad() or isWeapon:getSling() and TableHasValue(SingleShotRiflesTable, weaponScript:getName())) then
                    local tooltip = ISInventoryPaneContextMenu.addToolTip();
                    option.notAvailable = true;
                    tooltip:setName(isWeapon:getSling():getName());
                    tooltip.description = tooltip.description .. getText("Tooltip_remove_Forearm_Warning");
                    option.toolTip = tooltip;
                end
            end
            if isWeapon:getSling() then
                local option = subMenuRemove:addOption(isWeapon:getSling():getName(), isWeapon, ISInventoryPaneContextMenu.onRemoveUpgradeWeapon, isWeapon:getSling(), getSpecificPlayer(player));
                local weaponScript = isWeapon:getScriptItem();
                if (isWeapon:getClip() or isWeapon:getCanon() or isWeapon:getRecoilpad() and TableHasValue(AutomaticRiflesTable, weaponScript:getName())) then
                    local tooltip = ISInventoryPaneContextMenu.addToolTip();
                    option.notAvailable = true;
                    tooltip:setName(isWeapon:getSling():getName());
                    tooltip.description = tooltip.description .. getText("Tooltip_remove_Handguard_Warning");
                    option.toolTip = tooltip;
                end
            end
        ...
        if not instanceof(item, "HandWeapon") and item:getAmmoType() == bullet:getFullType() then
            if item:getCurrentAmmoCount() < item:getMaxAmmo() then
                ...
                tooltip.description =
                        ...
                        getText("ContextMenu_GunType") .. ": " .. getText(item:getDisplayName()) .. "\n" ..
