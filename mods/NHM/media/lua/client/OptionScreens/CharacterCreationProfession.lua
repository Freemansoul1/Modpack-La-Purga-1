...
--PLAYER_DEAD = false
nestedTraitsList = {"Choppa", "Swordway", "Shooter", "Cuttingedge", "Flurry", "Ironlungs", "Longshaft", "Precision", "Heartstrike"}
...
function CharacterCreationProfession:addTrait(bad)
    ...
    local function CheckForTraitSelected(selectedTraitList, SelectedTrait)
        for i = 1, selectedTraitList:size() do
            if selectedTraitList.items[i].text == SelectedTrait then
                print(tostring(SelectedTrait) .. ' found!')
                return true
            end
        end
        print(tostring(SelectedTrait) .. ' not found sus!')
        return false
    end

    local function CheckForTrait(TraitList, Trait)
        for i = 1, TraitList:size() do
            if TraitList.items[i].text == Trait then
                print(tostring(Trait) .. ' found!')
                return true
            end
        end
        print(tostring(Trait) .. ' not found sus!')
        return false
    end
    ...
    -- New Hope Additional Traits Init

    local traitchoppa = TraitFactory.getTrait("Choppa");
    local traitSwordway = TraitFactory.getTrait("Swordway");
    local traitShooter = TraitFactory.getTrait("Shooter");
    local traitCuttingedge = TraitFactory.getTrait("Cuttingedge");
    local traitHeartstrike = TraitFactory.getTrait("Heartstrike");
    local traitFlurry = TraitFactory.getTrait("Flurry");
    local traitIronlungs = TraitFactory.getTrait("Ironlungs");
    local traitLongshaft = TraitFactory.getTrait("Longshaft");
    local traitPrecision = TraitFactory.getTrait("Precision");

    for i = 1, self.listboxTraitSelected:size() do

        local selectedTraitNH = self.listboxTraitSelected.items[i].item
        local TraitListNH = self.listboxTrait
        local TraitListSelectedNH = self.listboxTraitSelected

        --HunterNH -> + Shooter
        if selectedTraitNH:getType() == "HunterNH" and CheckForTraitSelected(TraitListSelectedNH, traitShooter:getLabel()) == false and CheckForTrait(TraitListNH, traitShooter:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitShooter:getLabel(), traitShooter)
            NewTraitNH.tooltip = traitShooter:getDescription();
        end

        --Shooter -> + Precision
        if selectedTraitNH:getType() == "Shooter" and CheckForTraitSelected(TraitListSelectedNH, traitPrecision:getLabel()) == false and CheckForTrait(TraitListNH, traitPrecision:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitPrecision:getLabel(), traitPrecision)
            NewTraitNH.tooltip = traitPrecision:getDescription();
        end

        --harpoon -> + Longshaft
        if selectedTraitNH:getType() == "harpoon" and CheckForTraitSelected(TraitListSelectedNH, traitLongshaft:getLabel()) == false and CheckForTrait(TraitListNH, traitLongshaft:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitLongshaft:getLabel(), traitLongshaft)
            NewTraitNH.tooltip = traitLongshaft:getDescription();
        end

        --swordsman -> + Cuttingedge
        if selectedTraitNH:getType() == "swordsman" and CheckForTraitSelected(TraitListSelectedNH, traitCuttingedge:getLabel()) == false and CheckForTrait(TraitListNH, traitCuttingedge:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitCuttingedge:getLabel(), traitCuttingedge)
            NewTraitNH.tooltip = traitCuttingedge:getDescription();
        end

        --Cuttingedge -> + Swordway
        if selectedTraitNH:getType() == "Cuttingedge" and CheckForTraitSelected(TraitListSelectedNH, traitSwordway:getLabel()) == false and CheckForTrait(TraitListNH, traitSwordway:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitSwordway:getLabel(), traitSwordway)
            NewTraitNH.tooltip = traitSwordway:getDescription();
        end

        --CockNH -> + Heartstrike
        if selectedTraitNH:getType() == "CockNH" and CheckForTraitSelected(TraitListSelectedNH, traitHeartstrike:getLabel()) == false and CheckForTrait(TraitListNH, traitHeartstrike:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitHeartstrike:getLabel(), traitHeartstrike)
            NewTraitNH.tooltip = traitHeartstrike:getDescription();
        end

        --Heartstrike -> + Flurry
        if selectedTraitNH:getType() == "Heartstrike" and CheckForTraitSelected(TraitListSelectedNH, traitFlurry:getLabel()) == false and CheckForTrait(TraitListNH, traitFlurry:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitFlurry:getLabel(), traitFlurry)
            NewTraitNH.tooltip = traitFlurry:getDescription();
        end

        --BrawlerNH -> + Ironlungs
        if selectedTraitNH:getType() == "BrawlerNH" and CheckForTraitSelected(TraitListSelectedNH, traitIronlungs:getLabel()) == false and CheckForTrait(TraitListNH, traitIronlungs:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitIronlungs:getLabel(), traitIronlungs)
            NewTraitNH.tooltip = traitIronlungs:getDescription();
        end

        --Miner -> + Choppa
        if selectedTraitNH:getType() == "miner" and CheckForTraitSelected(TraitListSelectedNH, traitchoppa:getLabel()) == false and CheckForTrait(TraitListNH, traitchoppa:getLabel()) == false then
            local NewTraitNH = self.listboxTrait:addItem(traitchoppa:getLabel(), traitchoppa)
            NewTraitNH.tooltip = traitchoppa:getDescription();
        end
...
function CharacterCreationProfession:removeTrait()
    ...
    local selectedTraitListNH = self.listboxTraitSelected
    ...
    if not trait:isFree() then
        ...
        -- New Hope Additional Traits Deletion

        if trait:getType() == "miner" then
            -- remove Longshaft
            if CheckForTraitSelected(selectedTraitListNH, traitchoppa:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitchoppa:getLabel())
                self.pointToSpend = self.pointToSpend + traitchoppa:getCost();
            else
                self.listboxTrait:removeItem(traitchoppa:getLabel())
            end
        end

        if trait:getType() == "harpoon" then
            -- remove Longshaft
            if CheckForTraitSelected(selectedTraitListNH, traitLongshaft:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitLongshaft:getLabel())
                self.pointToSpend = self.pointToSpend + traitLongshaft:getCost();
            else
                self.listboxTrait:removeItem(traitLongshaft:getLabel())
            end
        end

        if trait:getType() == "swordsman" then
            -- remove Cuttingedge
            if CheckForTraitSelected(selectedTraitListNH, traitCuttingedge:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitCuttingedge:getLabel())
                self.pointToSpend = self.pointToSpend + traitCuttingedge:getCost();
                if CheckForTraitSelected(selectedTraitListNH, traitSwordway:getLabel()) == true then
                    self.listboxTraitSelected:removeItem(traitSwordway:getLabel())
                    self.pointToSpend = self.pointToSpend + traitSwordway:getCost();
                else
                    self.listboxTrait:removeItem(traitSwordway:getLabel())
                end
            else
                self.listboxTrait:removeItem(traitCuttingedge:getLabel())
            end
        end

        if trait:getType() == "Cuttingedge" then
            -- remove Swordway
            if CheckForTraitSelected(selectedTraitListNH, traitSwordway:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitSwordway:getLabel())
                self.pointToSpend = self.pointToSpend + traitSwordway:getCost();
        ...
        if trait:getType() == "CockNH" then
            -- remove CockNH
            if CheckForTraitSelected(selectedTraitListNH, traitHeartstrike:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitHeartstrike:getLabel())
                self.pointToSpend = self.pointToSpend + traitHeartstrike:getCost();
                if CheckForTraitSelected(selectedTraitListNH, traitFlurry:getLabel()) == true then
                    self.listboxTraitSelected:removeItem(traitFlurry:getLabel())
                    self.pointToSpend = self.pointToSpend + traitFlurry:getCost();
                else
                    self.listboxTrait:removeItem(traitFlurry:getLabel())
                end
            else
                self.listboxTrait:removeItem(traitHeartstrike:getLabel())
            end
        end

        if trait:getType() == "Heartstrike" then
            -- remove Flurry
            if CheckForTraitSelected(selectedTraitListNH, traitFlurry:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitFlurry:getLabel())
                self.pointToSpend = self.pointToSpend + traitFlurry:getCost();
        ...
        if trait:getType() == "BrawlerNH" then
            -- remove Ironlungs
            if CheckForTraitSelected(selectedTraitListNH, traitIronlungs:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitIronlungs:getLabel())
                self.pointToSpend = self.pointToSpend + traitIronlungs:getCost();
            else
                self.listboxTrait:removeItem(traitIronlungs:getLabel())
            end
        end

        if trait:getType() == "HunterNH" then
            -- remove Shooter
            if CheckForTraitSelected(selectedTraitListNH, traitShooter:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitShooter:getLabel())
                self.pointToSpend = self.pointToSpend + traitShooter:getCost();
                if CheckForTraitSelected(selectedTraitListNH, traitPrecision:getLabel()) == true then
                    self.listboxTraitSelected:removeItem(traitPrecision:getLabel())
                    self.pointToSpend = self.pointToSpend + traitPrecision:getCost();
                else
                    self.listboxTrait:removeItem(traitPrecision:getLabel())
                end
            else
                self.listboxTrait:removeItem(traitShooter:getLabel())
            end
        end

        if trait:getType() == "Shooter" then
            -- remove Precision
            if CheckForTraitSelected(selectedTraitListNH, traitPrecision:getLabel()) == true then
                self.listboxTraitSelected:removeItem(traitPrecision:getLabel())
                self.pointToSpend = self.pointToSpend + traitPrecision:getCost();
...
function CharacterCreationProfession:isNested(trait)
    for i = 1, #nestedTraitsList do
        if trait:getType() == nestedTraitsList[i] then
            return true
        end
    end
    return false
...
function CharacterCreationProfession:populateTraitList(list)
    ...
    for i = 0, traitList:size() - 1 do
        ...
        if not trait:isFree() and trait:getCost() > 0 and ((trait:isRemoveInMP() and not isClient()) or not trait:isRemoveInMP()) and not CharacterCreationProfession:isNested(trait) then
...
--function CharacterCreationProfession:changePlayerState()
--    PLAYER_DEAD = true
--    print("Current player state is dead")
--end
...
--Events.OnPlayerDeath.Add(CharacterCreationProfession.changePlayerState)
