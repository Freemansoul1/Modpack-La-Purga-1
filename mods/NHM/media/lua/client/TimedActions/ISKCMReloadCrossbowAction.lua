...
function ISKCMReloadCrossbowAction:animEvent(event, parameter)
	...
	if event == 'unloadFinished' then
		self:unloadAmmo();
		if self.gun:getType() == "Bow" then self.gun:setWeaponSprite("Base.Bow"); end
		if self.gun:getType() == "CBow" then self.gun:setWeaponSprite("Base.CBow"); end
		if self.gun:getType() == "RBow" then self.gun:setWeaponSprite("Base.RBow"); end
	...
	if event == 'loadFinished' then
		...
		if self.gun:getType() == "Bow" then self.gun:setWeaponSprite("Base.Bow3"); end
		if self.gun:getType() == "CBow" then self.gun:setWeaponSprite("Base.CBow3"); end
		if self.gun:getType() == "RBow" then self.gun:setWeaponSprite("Base.RBow3"); end
		self.character:resetEquippedHandsModels();
	end
