...
function ISRemoveBush:start()
	...
	Woodcutting.noise("Max time = ".. self.maxTime)
...
function ISRemoveBush:animEvent(event, parameter)
	if event == 'Chop' then
		...
		if self.perkLevel < 8 or self.character:getStats():getEndurance() > 0.26 then
			self:useEndurance()
		end
		local chanceToLowerConditionOneIn = self.weapon:getConditionLowerChance() * 4 +self.perkLevel
		if self.weapon and ZombRand(chanceToLowerConditionOneIn) == 0 then
			self.weapon:setCondition(self.weapon:getCondition() - 1)
			ISWorldObjectContextMenu.checkWeapon(self.character)
			self.character:getXp():AddXP(Perks.Woodcutting, 0.2*Woodcutting.Settings.xpMultiplier)
		else
...
function ISRemoveBush:perform()
	...
	if not self.character:getModData().bushkills then
		self.character:getModData().bushkills = 1
	else
		self.character:getModData().bushkills = self.character:getModData().bushkills + 1
	end
...
function ISRemoveBush:new(character, square, wallVine)
	...
	o.perkLevel = character:getPerkLevel(Perks.Woodcutting)
	o.square = square
	o.stopOnWalk = true
	o.stopOnRun = true
	o.maxTime = 100 - o.perkLevel*6
	o.spriteFrame = 0
	o.wallVine = wallVine
