...
ISScrollingCheckListBox = ISPanelJoypad:derive("ISScrollingCheckListBox");
ISScrollingCheckListBox.joypadListIndex = 1;
...
function ISScrollingCheckListBox:initialise()
    ISPanelJoypad.initialise(self);
end

function ISScrollingCheckListBox:setJoypadFocused(focused, joypadData)
    if focused then
...
function ISScrollingCheckListBox:onJoypadDirRight(joypadData)
    if self.joypadParent then
        self.joypadParent:onJoypadDirRight(joypadData);
    end
end

function ISScrollingCheckListBox:onJoypadDirLeft(joypadData)
    if self.joypadParent then
...
--**
--************************************************************************--
function ISScrollingCheckListBox:instantiate()
...
function ISScrollingCheckListBox:rowAt(x, y)
	local y0 = 0
...
function ISScrollingCheckListBox:topOfItem(index)
	local y = 0
...
function ISScrollingCheckListBox:prevVisibleIndex(index)
...
function ISScrollingCheckListBox:nextVisibleItem(index)
...
ISScrollingCheckListBox.nextVisibleIndex = ISScrollingCheckListBox.nextVisibleItem

function ISScrollingCheckListBox:isMouseOverScrollBar()
	return self:isVScrollBarVisible() and self.vscroll:isMouseOver()
end

function ISScrollingCheckListBox:onMouseMove(dx, dy)
	if self:isMouseOverScrollBar() then return end
	self.mouseoverselected = self:rowAt(self:getMouseX(), self:getMouseY())
end

function ISScrollingCheckListBox:onMouseMoveOutside(x, y)
	self.mouseoverselected = -1;
end

function ISScrollingCheckListBox:onMouseUpOutside(x, y)
	if self.vscroll then
		self.vscroll.scrolling = false;
	end
end

function ISScrollingCheckListBox:onMouseUp(x, y)
	if self.vscroll then
		self.vscroll.scrolling = false;
	end
end

function ISScrollingCheckListBox:addItem(name, item)
    local i = {}
    i.text=name;
    i.item=item;
	i.tooltip = nil;
...
function ISScrollingCheckListBox:insertItem(index, name, item)
	local i = {}
	i.text = name
	i.item = item
	i.tooltip = nil
...
function ISScrollingCheckListBox:removeItem(itemText)
	for i,v in ipairs(self.items) do
...
function ISScrollingCheckListBox:removeItemId(itemId)
	for i,v in ipairs(self.items) do
		print(v.item .. "/" .. itemId)
...
function ISScrollingCheckListBox:removeItemByIndex(itemIndex)
...
function ISScrollingCheckListBox:removeFirst()
...
function ISScrollingCheckListBox:size()
    return self.count;
end

function ISScrollingCheckListBox:setOnMouseDownFunction(target, onmousedown)
	self.onmousedown = onmousedown;
	self.target = target;
end

function ISScrollingCheckListBox:setOnMouseDoubleClick(target, onmousedblclick)
	self.onmousedblclick = onmousedblclick;
	self.target = target;
end

function ISScrollingCheckListBox:drawTextWithLines(self, text, x, y, lineHeight, font, item)
    local startX = x
    local lines = {}
    ...
    -- 각 줄에 대해 텍스트와 중앙선 그리기
    for i, line in ipairs(lines) do
        local lineY = y + (i - 1) * lineHeight

        -- 중앙선 그리기
        --if #lines == 1 then lineHeight = item.height end
        local textWidth = getTextManager():MeasureStringX(font, line)
        self:drawRect(startX, lineY + lineHeight / 2 - 0.5, textWidth, 1, 1, 1, 0, 0)
    end
end

function ISScrollingCheckListBox:doDrawItem(y, item, alt)
	item.height = self.itemheight
	local SCW = 0
	if self:isVScrollBarVisible() then SCW = 20 end
	local wText = toDOU.wrapText(item.text, self.font, self.width-35-SCW)
	local txx = getTextManager():MeasureStringX(self.font, wText)
	local ty = getTextManager():MeasureStringY(self.font, wText)
	if ty > item.height then item.height = ty end
	local tx = 35
	local idx = toDOU.findItemByIndex(toDOU.getFindModData(toDO.playerObj), item.item)
	if idx ~= 0 and toDOU.getFindModData(toDO.playerObj)[idx].check then 
		--self:drawRect(35, y+item.height/2-0.5, txx, 1, 1, 1, 0, 0);
		local nx = getTextManager():MeasureStringX(self.font, toDOU.getFindModData(toDO.playerObj)[idx].checkName)
		local tyy = getTextManager():MeasureStringY(self.font, toDOU.getFindModData(toDO.playerObj)[idx].checkName)
		wText = toDOU.wrapText(item.text, self.font, self.width-60-nx-SCW)
		local tty =  getTextManager():MeasureStringY(self.font, wText)
		--print(tty .. " / " .. item.height)
		if tty >= item.height then 
			if tty > item.height then item.height = tty end
			if tty == item.height then item.height = tty + 16 end
			ty = tty
		end
		self:drawTextureScaledAspect(getTexture("Quest_Succeed"), 10, y+item.height/2 - 5, 10, 10,  1, 1, 1, 1); 
		self:drawTextWithLines(self, wText, 35, y+item.height/2 - ty/2, getTextManager():getFontHeight(self.font), self.font, item)
		self:drawTextureScaledAspect(getTexture("media/textures/check.png"), self.width-25-nx-SCW, y+item.height/2 - 10, 20, 20,  1, 1, 1, 1); 
		self:drawText(toDOU.getFindModData(toDO.playerObj)[idx].checkName, self.width-nx-5-SCW, y+item.height/2 - tyy/2, 0, 0.7, 0, 1, self.font); 
	end
	if self.selected == item.index then
        self:drawRect(0, (y), self:getWidth(), item.height-1, 0.3, 0.7, 0.35, 0.15);
    end
	if item.item ~= "date" then self:drawRectBorder(5, y+item.height/2 - 10 , 20, 20, 1.0, 0, 0, 0) end
	self:drawRectBorder(0, (y), self:getWidth(), item.height, 0.5, self.borderColor.r, self.borderColor.g, self.borderColor.b);
	self.textColor = {r=0, g=0, b=0, a=1};
	if item.item == "date" then 
		tx = 45
		self:drawTextureScaledAspect(getTexture("media/textures/cicon.png"), 5, y+item.height/2 - 16, 32, 32,  1, 1, 1, 1);  
		self.textColor = {r=0, g=0.7, b=0, a=1};
	end
	self:drawText(wText, tx, y+item.height/2 - ty/2, self.textColor.r, self.textColor.g, self.textColor.b, self.textColor.a, self.font);
	y = y + item.height;
	return y;

end
function ISScrollingCheckListBox:clear()
	self.items = {}
	self.selected = 1;
...
function ISScrollingCheckListBox:onMouseWheel(del)
...
function ISScrollingCheckListBox:scrollToSelected()

end

function ISScrollingCheckListBox.sortByName(a, b)
    return not string.sort(a.text, b.text);

end
function ISScrollingCheckListBox:sort()
    table.sort(self.items, ISScrollingCheckListBox.sortByName);
    for i,item in ipairs(self.items) do
        item.itemindex = i;
    end
end

function ISScrollingCheckListBox:updateTooltip()
	local row = -1
...
function ISScrollingCheckListBox:updateSmoothScrolling()
...
function ISScrollingCheckListBox:prerender()
...
function ISScrollingCheckListBox:onMouseDoubleClick(x, y)
...
function ISScrollingCheckListBox:onMouseDown(x, y)
	...
	self.selected = row;
    local squareSize = 20
	local idIndex = self.selected-self.dateCount

	for i = 0, self:getHeight() / squareSize do
        local sx = 10
        local sy = i * (squareSize + 5) -- Adjust vertical spacing

        if x >= sx and x <= sx + squareSize and y >= sy and y <= sy + squareSize then
			local idx = toDOU.findItemByIndex(toDOU.getFindModData(toDO.playerObj), self.items[self.selected].item)
			if idx ~= 0 then
               if toDOU.getFindModData(toDO.playerObj)[idx].check then 
				   toDOU.getFindModData(toDO.playerObj)[idx].check = false
				   toDOU.getFindModData(toDO.playerObj)[idx].checkName = ""
			      else
				   if toDOU.getFindModData(toDO.playerObj)[idx] then 
					  toDOU.getFindModData(toDO.playerObj)[idx].check = toDOU.getFindModData(toDO.playerObj)[idx].id 
					  toDOU.getFindModData(toDO.playerObj)[idx].checkName = toDOU.getPlayerName(toDO.playerObj)
				   end
			   end
			   ModData.transmit("SharedTDL")
			   toDOU.refresh(toDO.playerObj, "true")
			end
            return true
		end
    end
...
function ISScrollingCheckListBox:onJoypadDirUp()
...
function ISScrollingCheckListBox:onJoypadDirDown()
...
function ISScrollingCheckListBox:ensureVisible(index)
...
function ISScrollingCheckListBox:render()
...
function ISScrollingCheckListBox:onJoypadDown(button, joypadData)
...
function ISScrollingCheckListBox:onLoseJoypadFocus(joypadData)
...
function ISScrollingCheckListBox:setFont(font, padY)
...
function ISScrollingCheckListBox:addColumn(columnName, size)
...
--**
--************************************************************************--
function ISScrollingCheckListBox:new (x, y, width, height)
	local o = {}
	--o.data = {}
	...
	o.y = y;
	o:noBackground();
	o.backgroundColor = {r=0.98, g=0.953, b=0.882, a=0};
	o.borderColor = {r=0.4, g=0.4, b=0.4, a=0};
	o.altBgColor = {r=0.2, g=0.3, b=0.2, a=0 }
	o.listHeaderColor = {r=0.4, g=0.4, b=0.4, a=0};
