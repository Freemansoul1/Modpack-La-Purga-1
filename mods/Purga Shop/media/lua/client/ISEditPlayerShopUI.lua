...
function ISEditPlayerShopUI:render()
    ...
    self:drawText("Nombre de la Tienda", self:getWidth()*0.025, z, 1,1,1,1, UIFont.Small)

    z = z + FONT_HGT_SMALL + 2 * FONT_SCALE + self.nameEntry:getHeight() + 10 * FONT_SCALE
    self:drawText("Descripcion", self:getWidth()*0.025, z, 1,1,1,1, UIFont.Small)

    z = z + FONT_HGT_SMALL + 2 * FONT_SCALE + self.descriptionEntry:getHeight() + 10 * FONT_SCALE
    self:drawText("Objetos", self:getWidth()*0.025, z, 1,1,1,1, UIFont.Small)
...
function ISEditPlayerShopUI:create()
    ...
    self.descriptionEntry = ISTextEntryBox:new(self.shopData.description or "Describe tu tienda", padBottom, z, self:getWidth() - padBottom * 2, height)
    ...
    for i = 0, items:size() - 1 do
      ...
      if not self.itemList.itemPrices[item:getType()] and item:getFullType() ~= SandboxVars.PlayerShops.CurrencyItem then
        self.itemList.itemPrices[item:getType()] = "Cargando..."
        local row = self.itemList:addItem(item:getDisplayName(), item)
        row.priceEntry = ISTextEntryBox:new("Cargando...", self.itemList:getWidth() - 75 - self.itemList.vscroll.width, 0, 70, inset + FONT_HGT_SMALL + inset)
...
function ISEditPlayerShopUI:onOptionMouseDown(button, x, y)
  if button.internal == "CANCELAR" then
    self:close()
  elseif button.internal == "GUARDAR" then
