...
function ISShowPlayerShopUI:create()
    ...
    self.buyButton = ISButton:new(self.itemList:getWidth() - 75 - self.itemList.vscroll.width, 0, 70, FONT_HGT_SMALL + 8 * FONT_SCALE, "COMPRAR", self, ISShowPlayerShopUI.onOptionMouseDown)
    self.buyButton.internal = "COMPRAR"
...
function ISShowPlayerShopUI:onOptionMouseDown(button, x, y)
  if button.internal == "CANCELAR" then
    ...
    self:close()
  elseif button.internal == "COMPRAR" then
    if self.buyModal then
      self.buyModal:close()
    end
