...
function ISBuyModal:createChildren()
  ...
  self.buyButton = ISButton:new(self.width/2 - 50 - 10, buttonY, 50, btnHgt, "COMPRAR", self, ISBuyModal.onOptionMouseDown)
  ...
  self.cancelButton.internal = "CANCELAR"
...
function ISBuyModal:onOptionMouseDown(button, x, y)
  ...
  elseif button.internal == "COMPRAR" then
    if tonumber(self.quantityEntry:getText()) > self.container:getCountType(self.itemType) then
      self.noticeText = "Cantidad erronea."
    elseif self:hasCurrency() then
      self:doPayment()
      self:doPurchase()
      self:close()
    else
      self.noticeText = "No tienes suficiente dinero"
    end
  end
end
