...
function DryFishMod.Recipes_OnInitGlobalModData()
    if SandboxVars.DryFishMod.CraftingMagazine then
        local manager = getScriptManager()

        local recipeList = manager:getItem("DFM.DryFishModCraftMagazine"):getTeachedRecipes()
        for i=0,recipeList:size()-1 do
            local recipe = manager:getRecipe("DFM."..tostring(recipeList:get(i)))
            if recipe ~= nil then recipe:setNeedToBeLearn(true) end
        end
    end
end

Events.OnInitGlobalModData.Add(DryFishMod.Recipes_OnInitGlobalModData)
