...
local function ValidateHours(playerObj, hours)
	if playerObj:getHoursSurvived() >= hours then
		return true
	else
		return false
	end
...
function AFOpenHiddenBoxAction:perform()
    ...
    if ZombRand(1, 101) <= totalpercent then
		...
		if iR ~= 100 then
			local it = items[ZombRand(1, #items+1)]
			if extrachoice ~= nil then
				local rolls = ZombRand(1, #items+1);
				if rolls > 1 then
					for i = 0, rolls do
						it = items[ZombRand(1, #items+1)];
						self:AddItem(inv, it)
						--inv:AddItem(it);
					end
				else
...
function AFOpenHiddenBoxAction:AddItem(inv, it)
	local ValidateHours = true--ValidateHours(self.character, 7300)
	if it == "Base.KeyE" then
		if ValidateHours then
			local key = inv:AddItem(it);
			key:setKeyId(50008);
			self.character:playSound("EasterEgg");
		else
			it = "Base.LGClimatologyMag";
			inv:AddItem(it);
		end
	else
		inv:AddItem(it);
