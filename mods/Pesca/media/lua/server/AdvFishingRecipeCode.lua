...
function Recipe.OnCreate.NewCutFish(items, result, player)
	...
	if fish then
		...
		if fish:getType() == "Arapaima"
		  or fish:getType() == "Tarpon" 
		  or fish:getType() == "Carp" then
			local number = 2;
			if fish:getType() == "Arapaima" then
				number = 6;
			elseif fish:getType() == "Tarpon" then
				number = 4;
			end
			player:getInventory():AddItems("Base.FishScales", number);
		end
	end
end
