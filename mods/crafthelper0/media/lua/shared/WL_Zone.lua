...
function WL_Zone:isInZone(x, y, z)
    if x >= self.minX and x <= (self.maxX+1) and y >= self.minY and y <= (self.maxY+1) and z >= (self.minZ) and z <= (self.maxZ) then
        return true
    end
    return false
end
...
function WL_Zone:isNearZone(x, y, z, range)
    if x >= self.minX-range and x <= (self.maxX+1+range) and y >= self.minY-range and y <= (self.maxY+1+range) and z >= (self.minZ-range) and z <= (self.maxZ+range) then
        return true
    end
    return false
end
