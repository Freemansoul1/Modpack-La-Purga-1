...
ISDoubleTileDefense = ISBuildingObject:derive("ISDoubleTileDefense");

--************************************************************************--
--** ISDoubleTileDefense:new
--**
--************************************************************************--
function ISDoubleTileDefense:create(x, y, z, north, sprite)
...
function ISDoubleTileDefense:walkTo(x, y, z)
...
function ISDoubleTileDefense:setInfo(square, north, sprite)
...
function ISDoubleTileDefense:removeFromGround(square)
...
function ISDoubleTileDefense:new(name, sprite1, sprite2, northSprite1, northSprite2)
	local o = {};
	setmetatable(o, self); 
	self.__index = self;
	o:init();
...
local WoodBaseHealth = SandboxVars.ChevalDeFrise.WoodBaseHealth
local WoodScaleHealth = SandboxVars.ChevalDeFrise.WoodScaleHealth

function ISDoubleTileDefense:getHealth() 
	if not self or not self.player then return WoodBaseHealth; end
	local playerObj = getSpecificPlayer(self.player)
	local health = (playerObj:getPerkLevel(Perks.Woodwork) * WoodScaleHealth) 
	return WoodBaseHealth + health
end

function ISDoubleTileDefense:render(x, y, z, square)
...
function ISDoubleTileDefense:isValid(square)
	if not ISBuildingObject.isValid(self, square) then
		return false
    end 
	if not square or not square:isFreeOrMidair(true) then return false; end 
...
function ISDoubleTileDefense:getSquare2Pos(square, north)
	local x = square:getX()
	local y = square:getY()
	local z = square:getZ()
	if north then
...
function ISDoubleTileDefense:getSquare2(square, north)
