        ...
        for x=buildingDef:getX(), buildingDef:getX2() do
            for y=buildingDef:getY(), buildingDef:getY2() do
                ...
                if (square and SafeHouse.isSafeHouse(square, nil, true)) then
                    print ("[INFO] Defenders are not allowed to spawn in a safehouse or PVE zone.")
                    return false
                end
                if square and (((buildingDef:getX() < 13541 and buildingDef:getX2() > 12879) and (buildingDef:getY() < 7484 and buildingDef:getY2() > 6883))) or (((buildingDef:getX() < 11697 and buildingDef:getX2() > 11404) and (buildingDef:getY() < 7797 and buildingDef:getY2() > 7502))) then
                    print ("[INFO] Defenders are not allowed to spawn in a safehouse or PVE zone.")
                    return false
                end
            end
