...
--[[
if SandboxVars.CarWanna.ExperimentalTsarModSupport then
...
function Commands.spawnVehicle(player, args)
    ...
    local vehicleID = car:getScript():getName()
    ...
    do
        ...
        --print(partId)
        if part:getCategory() ~= "nodisplay" or (not SandboxVars.CarWanna.IgnoreNodisplay and part:getCategory() == "nodisplay") or (SandboxVars.CarWanna.ExperimentalTsarModSupport and ATA2TuningTable and ATA2TuningTable[vehicleID]) then
        --
                    ...
                    if partdata then
                        if part:getInventoryItem() and part:getInventoryItem():getFullType() == partdata["Item"] then
                           noise(partdata["Item"].." already installed.")
                            ...
                            --ExperimentalTsarModSupport
                            if SandboxVars.CarWanna.ExperimentalTsarModSupport and partdata["Model"] then
                                noise("Attemping to set tsar model for part")
                                part:getModData().tuning2 = {}
                                part:getModData().tuning2.model = partdata["Model"]
                        ...
                        --TsarATA Support
                        if SandboxVars.CarWanna.ExperimentalTsarModSupport and ATA2TuningTable and ATA2TuningTable[vehicleID] and ATA2TuningTable[vehicleID].parts[partId] then
                            part:getModData().tuning2 = {}
