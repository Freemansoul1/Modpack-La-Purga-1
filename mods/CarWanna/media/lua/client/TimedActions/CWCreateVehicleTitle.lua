...
function CWCreateVehicleTitle:perform()
    ...
    local vehicleID = self.vehicle:getScript():getName()
    local vehicleName = getText("IGUI_VehicleName"..vehicleID)
...
--[[        
        ...
        if not SandboxVars.CarWanna.IgnoreNodisplay or part:getCategory() ~= "nodisplay" or (SandboxVars.CarWanna.ExperimentalTsarModSupport and ATA2TuningTable and ATA2TuningTable[vehicleID] and ATA2TuningTable[vehicleID].parts[partId]) then
            ...
            if part:getItemType() and not part:getItemType():isEmpty() then
                if partItem then
                    ...
                    --Experimental Tsarlib2.0 ModSupport
                    if SandboxVars.CarWanna.ExperimentalTsarModSupport and part:getModData().tuning2 and part:getModData().tuning2.model then
                        pinkSlipPartData[partId]["Model"] = part:getModData().tuning2.model
                    end
