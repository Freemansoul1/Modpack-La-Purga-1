...
function ISMoveablesAction:isValidObject()
    ...
    if objects then
        for i = 0, objects:size() - 1 do
            ...
            if object then
                local modData = object:getModData() -- Fix this whole mess later. Yes. Two loops are required. Thanks game. - Braven
                if modData and modData.hasAftmComboLock then
                    return false
                end
            end
        end
    ...
    if objects then
        for i = 0, objects:size() - 1 do
            local object = objects:get(i);
            if object then
                if self.moveProps.object == object then
                    return true;
                end;
            end
        end;
...
function ISMoveablesAction:isValid()
    ...
    --LAND CLAIM
    if self.origSpriteName == LandClaimConfig.LCItemType then
        if self.mode == 'place' then
            if self.square:getZ() > 0 then
                self.character:setHaloNote("Land Claim placement can only be on the ground floor!");
                return false
            end
            if SafeHouse.hasSafehouse(self.character) then
                self.character:setHaloNote("Land Claim placement is invalid because the player is already part of a safehouse!");
                return false
            end
            if not SafehouseController.IsSquareValidForNewSafehouse(self.square) then
                self.character:setHaloNote("Land Claim placement is invalid because there is already another safehouse nearby!");
                return false
            end
        elseif self.mode == 'pickup' or self.mode == 'scrap' or self.mode == 'rotate' then
            if SafeHouse.getSafeHouse(self.square) and not SafehouseController.IsOwner(self.character, self.square) then
                self.character:setHaloNote("You have no rights to interact with this Land Claim!");
                return false
            end
        end
    end
    ...
    if isClient() and SafeHouse.isSafeHouse(self.square, self.character:getUsername(), true) then
        ...
        if self.mode == "pickup" and not SafehouseController.CanPickup(self.character, self.square) then
            self.character:setHaloNote("You have no rights to pick up tiles in this Land Claim!");
            self:stop();
            return false;
        end

        if self.mode == "scrap" and not SafehouseController.CanDismantle(self.character, self.square) then
            self.character:setHaloNote("You have no rights to dismantle in this Land Claim!");
            self:stop();
            return false;
        end
...
function ISMoveablesAction:perform()
    ...
    if self.origSpriteName == LandClaimConfig.LCItemType then
        if self.mode == 'place' then
            SafehouseController.CreateSafehouse(self.square, self.character)
        elseif SafeHouse.getSafeHouse(self.square) and self.mode == 'pickup' or self.mode == 'scrap' then
            SafehouseController.DeleteSafehouse(self.square, self.character)
        end
    end
