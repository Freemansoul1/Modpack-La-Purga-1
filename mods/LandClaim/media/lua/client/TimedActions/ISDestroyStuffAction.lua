...
function ISDestroyStuffAction:isValid()
	...
	if not SafehouseController.CanDestroy(self.character, self.item:getSquare()) then
		self.character:setHaloNote("You don't have permission to destroy in this Land Claim.");
		return false
	end
...
function ISDestroyStuffAction:perform()
	...
	if not self.item:getSquare():getWall(false) == self.item and not self.item:getSquare():getWall(true) == self.item then
		self.cornerCounter = -1
	elseif self.item:getSquare():getWall(false) == self.item and not self.item:getSquare():getWall(true) == self.item then
		self.cornerCounter = 0
	elseif not self.item:getSquare():getWall(false) == self.item and self.item:getSquare():getWall(true) == self.item then
		self.cornerCounter = 1
	end
