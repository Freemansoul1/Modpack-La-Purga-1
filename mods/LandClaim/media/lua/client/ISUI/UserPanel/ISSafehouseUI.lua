...
--**	      Overridden by monkey for Land Claim          **
--***********************************************************
...
function ISSafehouseUI:initialise()
    ...
    self.releaseSafehouse = ISButton:new(10, 0, 70, btnHgt, "Delete Land Claim", self, ISSafehouseUI.onClick);
    ...
    self.promotePlayer = ISButton:new(0, self.playerList.y + self.playerList.height + 5, 70, btnHgt2, getText("Promote"), self, ISSafehouseUI.onClick);
    self.promotePlayer.internal = "PROMOTE";
    self.promotePlayer:initialise();
    self.promotePlayer:instantiate();
    self.promotePlayer.borderColor = self.buttonBorderColor;
    self.promotePlayer:setWidthToTitle(70)
    self.promotePlayer:setX(self.playerList:getRight() - self.promotePlayer.width - self.promotePlayer.width)
    self:addChild(self.promotePlayer);
    self.promotePlayer.enable = false;
    self.promotePlayer:setVisible(self:isOwner() or self:hasPrivilegedAccessLevel());

    self.demotePlayer = ISButton:new(0, self.playerList.y + self.playerList.height + 5, 70, btnHgt2, getText("Demote"), self, ISSafehouseUI.onClick);
    self.demotePlayer.internal = "DEMOTE";
    self.demotePlayer:initialise();
    self.demotePlayer:instantiate();
    self.demotePlayer.borderColor = self.buttonBorderColor;
    self.demotePlayer:setWidthToTitle(70)
    self.demotePlayer:setX(self.playerList:getRight() - self.demotePlayer.width - self.demotePlayer.width - self.demotePlayer.width)
    self:addChild(self.demotePlayer);
    self.demotePlayer.enable = false;
    self.demotePlayer:setVisible(self:isOwner() or self:hasPrivilegedAccessLevel());
...
--        if self.safehouse:getPlayers():get(i) ~= self.player:getUsername() then
            ...
            if newPlayer.name ~= self.safehouse:getOwner() then
                local rank = SafehouseController.GetPlayerRank(self.safehouse, newPlayer.name);
                local name = newPlayer.name
                if rank ~= '' then
                    name = newPlayer.name .. ' (' .. rank .. '/' .. LandClaimConfig.RankLevels[rank] .. ')'
                end
                self.playerList:addItem(newPlayer.name, {name = name, username = newPlayer.name});
            end;
--        end
    end;
...
function ISSafehouseUI:render()
    ...
    if self.playerList.selected > 0 then
        ...
        self.selectedPlayer = self.playerList.items[self.playerList.selected].item.username;

        if SafehouseController.CanPromotePlayer(self.safehouse, self.selectedPlayer) then
            self.promotePlayer.enable = true;
        end

        if SafehouseController.CanDemotePlayer(self.safehouse, self.selectedPlayer) then
            self.demotePlayer.enable = true;
        end
...
function ISSafehouseUI:onClick(button)
    ...
    if button.internal == "RELEASE" then
        local modal = ISModalDialog:new(0,0, 350, 150, getText("IGUI_SafehouseUI_ReleaseConfirm", self.selectedPlayer)  .. " This will also delete the Land Claim.", true, nil, ISSafehouseUI.onReleaseSafehouse);
        modal:initialise()
    ...
    if button.internal == "PROMOTE" then
        SafehouseController.PromotePlayer(self.safehouse, self.selectedPlayer)
        self:populateList();
    end

    if button.internal == "DEMOTE" then
        SafehouseController.DemotePlayer(self.safehouse, self.selectedPlayer)
        self:populateList();
    end
end
...
function ISSafehouseUI:onRemovePlayerFromSafehouse(button, player)
    if button.internal == "YES" then
        ...
        --todo remove player from safehouse mod data
        SafehouseController.RemovePlayer(button.parent.ui.safehouse, button.parent.ui.selectedPlayer)
...
function ISSafehouseUI:onReleaseSafehouse(button, player)
    if button.internal == "YES" then
        if button.parent.ui:isOwner() or button.parent.ui:hasPrivilegedAccessLevel() then
            local player = getPlayerFromUsername(button.parent.ui.safehouse:getOwner())
            button.parent.ui.safehouse:removeSafeHouse(player);
    
            local safehouses = ClientData.GetLandClaimSafehouses()
            local safehouseData = safehouses[LC_Utils.GetSafehouseId(button.parent.ui.safehouse)]
            local square = getCell():getOrCreateGridSquare(safehouseData.Center.X, safehouseData.Center.Y, safehouseData.Center.Z)

            SafehouseController.DeleteSafehouseBySafehouse(button.parent.ui.safehouse, player)

            for i = 0, square:getObjects():size() - 1 do
                local landClaim = square:getObjects():get(i)
                local sprite = landClaim:getSprite()
        
                if sprite:getName() == LandClaimConfig.LCItemType then
                    square:transmitRemoveItemFromSquare(landClaim)
                    square:RemoveTileObject(landClaim)
                    SafehouseController.DeleteSafehouse(square, player)
                    player:setHaloNote("Land Claim destroyed!");
                    return
                end
            end
        end;
...
function ISSafehouseUI:onAnswerSafehouseInvite(button)
    ...
    if button.internal == "YES" then
        --Land Claim
        SafehouseController.AddPlayer(button.parent.safehouse, getPlayer():getUsername())
...
ISSafehouseUI.RefreshPlayerList = function()
    if ISSafehouseUI.instance and ISSafehouseUI.instance:isVisible() then
        ISSafehouseUI.instance:populateList();
    end
end
...
Events.AcceptedSafehouseInvite.Add(ISSafehouseUI.AcceptedSafehouseInvite);LuaEventManager.AddEvent("LC_ClientData_OnReceiveGlobalModData_Update")

if not Events.LC_ClientData_OnReceiveGlobalModData_Update then 
    LuaEventManager.AddEvent("LC_ClientData_OnReceiveGlobalModData_Update")
end

Events.LC_ClientData_OnReceiveGlobalModData_Update.Add(ISSafehouseUI.RefreshPlayerList)