...
function ISBuildAction:isValid()
    if SafeHouse.getSafeHouse(self.square) and (not SafehouseController.IsPlayerAllowedBySquare(self.character, self.square) or 
    not SafehouseController.CanBuild(self.character, self.square) or 
    not SafehouseController.IsSquareValidForBuilding(self.character, self.square)) then
        self.character:setHaloNote("You are not allowed to build here due to a Land Claim!");
        return false
    end
