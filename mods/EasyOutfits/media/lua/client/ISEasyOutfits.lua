...
ISEasyOutfits.GetModData = function(player, container)
	...
	if parent then
		...
		outfitModData = modData.easyOutfits;
	else
		local modData = player:getModData();
		if modData.easyOutfits == nil then
			modData.easyOutfits = {};			
		end
		local key = container:getContainingItem():getID();
		if modData.easyOutfits[key] == nil then
			modData.easyOutfits[key] = {};
			modData.easyOutfits[key].enable = false;
			modData.easyOutfits[key].outfits = {};
		end
		--object = bag;
		outfitModData = modData.easyOutfits[key];
	end
	return object, outfitModData;
end
...
-- ISCharacterScreen.loadFavouriteWeapon = function(self)
	...
	-- local swing = 0;
	-- for key,data in pairs(self.char:getModData()) do
		-- print("The key is " .. key);
		-- for index in string.gmatch(key, "^Fav:(.+)") do
			-- if data > swing then
...
-- ISEasyOutfits.FixFavWeapon = function(player, item)
	-- local modData = player:getModData();
...
ISEasyOutfits.TagContextMenu = function(player, context, items)
	...
	for _, item in ipairs(items) do
		if item:getFullType() == "EasyOutfits.OutfitTag" then
			...
			--local fixOption = context:addOption(getText("ContextMenu_EasyOutfitsFixFavoriteWeapon"), playerObj, ISEasyOutfits.FixFavWeapon, item);
			break;
		end
	end
end
	...
	if container then
		if outfit.outfitRegistered then
			...
			for i=0, player:getWornItems():size()-1 do
				...
				if (itemToBeStored:getCategory() == "Clothing") 
					and (itemToBeStored:getDisplayCategory() ~= "Bandage") 
					and (itemToBeStored:getDisplayCategory() ~= "ZedDmg") 
					and (itemToBeStored:getDisplayCategory() ~= "Wound") then
					ISTimedActionQueue.add(ISInventoryTransferAction:new(player, itemToBeStored, itemToBeStored:getContainer(), container, ISEasyOutfits.ACTIONTIME));
				end
			end
		end
...
ISEasyOutfits.RegisterOutfit = function(player, container, outfit)	
	if container then
		...
		for i=items:size()-1, 0, -1 do
			...
			if (itemToRegister:getCategory() == "Clothing") 
				and (itemToRegister:getDisplayCategory() ~= "Bandage") 
				and (itemToRegister:getDisplayCategory() ~= "ZedDmg") 
				and (itemToRegister:getDisplayCategory() ~= "Wound") then
...
ISEasyOutfits.StoreRegisterOutfit = function(player, container, outfit)
	if container then
		for i=0, player:getWornItems():size()-1 do
			local itemToBeStored = player:getWornItems():getItemByIndex(i)
			if (itemToBeStored:getCategory() == "Clothing") 
				...
				and (itemToBeStored:getDisplayCategory() ~= "ZedDmg") 
				and (itemToBeStored:getDisplayCategory() ~= "Wound") then
				table.insert(outfit.outfitItems, itemToBeStored:getID());
				outfit.outfitRegistered = true;
				ISTimedActionQueue.add(ISInventoryTransferAction:new(player, itemToBeStored, itemToBeStored:getContainer(), container, ISEasyOutfits.ACTIONTIME));
			end
		end
...
ISEasyOutfits.WorldOutfitsContextMenu = function(player, context, worldobjects, test)
	...
	for obj,_ in pairs(dupCheck) do
		...
		if modData and modData.enable then
			local useContext = context;
			if #modData.outfits > 1 then
				local outfitSubMenu = context:getNew(context);
				context:addSubMenu(context:addOption("ContextMenu_EasyOutfitsOutfits", outfitSubMenu));
				useContext = outfitSubMenu;
			end
			for name,outfit in pairs(modData.outfits) do
				local optionContainer = useContext:getNew(useContext);			
				local containerName = name;
				useContext:addSubMenu(useContext:addOption(containerName), optionContainer);
				...
				local optionStoreRegister = optionContainer:addOption(getText("ContextMenu_EasyOutfitsStoreAndRegisterOutfit"), playerObj, ISEasyOutfits.StoreRegisterOutfit, container, outfit);
...
ISEasyOutfits.InventoryOutfitsContextMenu = function(player, context, items)
	...
	for _, item in ipairs(items) do
		...
		local outfitsModData = playerObj:getModData().easyOutfits;
		if outfitsModData then
			if outfitsModData[itemId] then
				local modData = outfitsModData[itemId];
				if modData and modData.enable then
					local useContext = context;
					if #modData.outfits > 1 then
						local outfitSubMenu = context:getNew(context);
					...
					for name,outfit in pairs(modData.outfits) do
						...
						local optionStoreRegisterOutfit = optionContainer:addOption(getText("ContextMenu_EasyOutfitsStoreAndRegisterOutfit"), playerObj, ISEasyOutfits.StoreRegisterOutfit, container, outfit);
