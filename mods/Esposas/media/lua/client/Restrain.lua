...
local originalFillMenu = ISEmoteRadialMenu.fillMenu

function ISEmoteRadialMenu:fillMenu()
    -- Llamar a la función original para no perder otros emotes
    originalFillMenu(self)

    -- Añadir el emote personalizado "surrender"
    self:addOption("Surrender", self.player, self.onEmote, "surrender")

    -- Depuración para asegurarnos de que el emote fue añadido
    print("DEBUG: Emote 'surrender' añadido al menú radial.")
end
...
-- Función que sobrescribe el comportamiento del emote "surrender" de forma segura
local function overrideEmoteRadialMenu()
    local upperLayerEmote = ISEmoteRadialMenu.emote

    -- Sobrescribir la función emote del menú radial
    function ISEmoteRadialMenu:emote(emote)
        if emote == "surrender" then
            ...
            -- Llamar a la función original para cualquier otro emote
            upperLayerEmote(self, emote)
        end
    end
end

-- Función para añadir el emote "surrender" si no está presente
local function addSurrenderEmoteToRadial()
    -- Obtener el menú radial
    local emoteRadialMenu = getPlayer():getModData().emoteRadialMenu

    -- Verificar si el emote "surrender" ya está registrado
    local hasSurrender = false
    for _, emote in ipairs(emoteRadialMenu) do
        if emote == "surrender" then
            hasSurrender = true
            break
        end
    ...
    -- Si no está, añadir el emote "surrender"
    if not hasSurrender then
        table.insert(emoteRadialMenu, "surrender")
        print("DEBUG: Emote 'surrender' añadido al menú radial.")
    end
end

-- Evento que sobrescribe y añade el emote después de que se haya inicializado el menú radial
Events.OnGameStart.Add(function()
    overrideEmoteRadialMenu()
    addSurrenderEmoteToRadial()
end)
