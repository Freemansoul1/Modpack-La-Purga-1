...
function ISPostDeathUI:createChildren()
    ...
    local labelWid = buttonWid
    local labelHgt = 25
    local labelX = 0
    local labelY = 0
    local label = ISButton:new(labelX, labelY, labelWid, labelHgt, "", nil)
    label:initialise()
    label.backgroundColor.a = 0
    label.borderColor.a = 0
    label:ignoreWidthChange()
    label:ignoreHeightChange()
    label:setAnchorRight(false)
    label:setAnchorLeft(false)
    label:setAnchorTop(false)
    label:setAnchorBottom(false)
    label:setFont(UIFont.Large)
    self:addChild(label)
    ...
    -- Set the start time and duration of the timer
    self.timerStart = getTimestamp()
    self.timerDuration = SandboxVars.RaidsPurga.Countdown
end

function ISPostDeathUI:update()
    ISUIElement.update(self)
    
    -- Calculate the time remaining in seconds
    local timeElapsed = getTimestamp() - self.timerStart
    local timeRemaining = math.max(0, self.timerDuration - timeElapsed)

    -- Update the timer label text
    local timerText = string.format("Nuevo Personaje Disponible En %02d:%02d", math.floor(timeRemaining/60), timeRemaining%60)
    self.timerLabel:setTitle(timerText)

    -- Check if the timer has expired
    if timeElapsed >= self.timerDuration then
        self:onRespawn()
    end
end
...
function ISPostDeathUI:prerender()
    ...
    local timeElapsed = getTimestamp() - self.timerStart
    local timeRemaining = math.max(0, self.timerDuration - timeElapsed)
    local allowRespawnTimerExpired = timeElapsed >= self.timerDuration
    self.buttonRespawn:setVisible(self.waitOver and allowRespawn and allowRespawnTimerExpired)
