...
function RecycleVehicleAction:perform()
    ...
    local metalSheetSmall = math.floor((self.vArea - metalSheet * 3) / 3)
    local metalBar = math.floor(self.vArea - metalSheet * 3 - metalSheetSmall * 2)
    -- Other elements
    local metalPipe = 1
    local metalScrap = 2
    ...
    if vehicleName == "VanRadio" or vehicleName == "VanAmbulance" then
        electronicsScrap = electronicsScrap + 3 * 0.5
        electricWire = electricWire + 3 * 0.5
    ...
    for i = 1, self.vehicle:getPartCount() do
        ...
        if partItem then
            ...
            if partId:find("Brake") then
                metalUnusable = metalUnusable + 1 * 0.25
            end
            ...
            if partItemCondition > 30 then
                if partId:find("Door") then
                    if partItemCondition > 70 then
                        metalSheet = metalSheet + 1 * 0.25
                    else
                        metalSheetSmall = metalSheetSmall + 1 * 0.25
                    end
                elseif partId:find("GasTank") then
                    if partItemCondition > 70 then
                        if partItemType:find("^Small") then
                            metalSheetSmall = metalSheetSmall + 2 * 0.25
                        elseif partItemType:find("^Normal") then
                            metalSheet = metalSheet + 1 * 0.25
                        elseif partItemType:find("^Big") then
                            metalSheet = metalSheet + 2 * 0.25
                        end
                    else
                        metalSheetSmall = metalSheetSmall + 1 * 0.25
                    end
                elseif partId:find("Muffler") then
                    if partItemCondition > 70 then
                        metalSheetSmall = metalSheetSmall + 1 * 0.25
                        metalPipe = metalPipe + 2 * 0.25
                    else
                        metalPipe = metalPipe + 1 * 0.25
                    end
                ...
                elseif partId:find("Radio") then
                    electronicsScrap = electronicsScrap + 1* 0.25
                    electricWire = electricWire + 1* 0.25
                elseif partId:find("Seat") then
                    fabric = fabric + 3 * 0.25
                end
            else
    ...
    if RecycleVehicle.Utils.isSmashed(self.vehicle) then
        metalSheet = math.floor(metalSheet * 0.5 + 0.5)
        metalSheetSmall = math.floor(metalSheetSmall * 0.5 + 0.5)
        metalBar = math.floor(metalBar * 0.5 + 0.5)
    end
