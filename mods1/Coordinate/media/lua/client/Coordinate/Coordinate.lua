...
local Coordinate = {}
Coordinate.NAME = 'Coordinate Show'
Coordinate.AUTHOR = 'ProjectSky'
Coordinate.VERSION = '0.0.6'
...
print('Mod Loaded: ' .. Coordinate.NAME .. ' by ' .. Coordinate.AUTHOR .. ' (v' .. Coordinate.VERSION .. ')')

local render = ISCharacterScreen.render

function ISCharacterScreen:render()
  if not SandboxVars.Character.DisplayLocationEnable then
    render(self)
    return
  end
...
--    self:drawText(round(self.char:getNutrition():getCalories(), 2) .. "", self.xOffset + 10, z, 1,1,1,0.5, UIFont.Small);
	...
	self:drawTextRight(getText('IGUI_Loc_String'), x, z, 1, 1, 1, 1, UIFont.Small)
	self:drawText(format('%04d,%04d,%01d', self.char:getX(), self.char:getY(), self.char:getZ()), x + 10, z, 1, 1, 1, 0.5, UIFont.Small)
