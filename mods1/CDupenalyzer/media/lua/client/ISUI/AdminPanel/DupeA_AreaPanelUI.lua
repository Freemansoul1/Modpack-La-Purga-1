DupeA = DupeA or {}
ISAddDupeZoneUI = ISPanel:derive("ISAddDupeZoneUI")

local FONT_HGT_SMALL = getTextManager():getFontHeight(UIFont.Small)
local FONT_HGT_MEDIUM = getTextManager():getFontHeight(UIFont.Medium)

function ISAddDupeZoneUI:initialise()
    ...
    self.cancel = ISButton:new(self:getWidth() - btnWid - 10, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Cancel"), self, ISAddDupeZoneUI.onClick)
    ...
    self.ok = ISButton:new(10, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("IGUI_DupeAnalyzer_Select"), self, ISAddDupeZoneUI.onClick)
    self.ok.internal = "OK"
    ...
    self.defineStartingPointBtn = ISButton:new(10, 0, btnWid, btnHgt2, getText("IGUI_PvpZone_RedefineStartingPoint"), self, ISAddDupeZoneUI.onClick)
    ...
    local title = "Dupe Zone #" .. NonPvpZone.getAllZones():size() + 1
...
function ISAddDupeZoneUI:prerender()
    local z = 20
...
function ISAddDupeZoneUI:updateButtons()
    self.ok.enable = self.size > 1 and self.size < 250
end

function ISAddDupeZoneUI:addCoords(player, text, startingX, startingY)
    DupeA.coords = {}
    local x1, y1, x2, y2 = startingX, startingY, player:getX(), player:getY()
    local newEntry = {startX=x1, startY=y1, endX=x2, endY=y2}
    DupeA.coords[text] = newEntry
end

function ISAddDupeZoneUI:analyzeCoords(coords, title, player)
    for _, coord in pairs(coords) do
        DupeA.dupes = DupeA.AnalyzeItems(coord, title, player)
    end
end

function ISAddDupeZoneUI:onClick(button)
    if button.internal == "OK" then
        local doneIt = true
        if doneIt then
            self:setVisible(false)
            ISAddDupeZoneUI:addCoords(self.player, self.titleEntry:getInternalText(), self.startingX, self.startingY)
            ISAddDupeZoneUI:analyzeCoords(DupeA.coords, self.titleEntry:getInternalText(), self.player)
...
function ISAddDupeZoneUI:new(x, y, width, height, player)
    local o = {}
