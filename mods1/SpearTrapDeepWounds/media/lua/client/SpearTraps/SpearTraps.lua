...
local function doFall(player)
  print("Attempting to force a trip...")
  player:clearVariable("BumpFallType")
  player:setBumpType("stagger")
  player:setBumpDone(false)
  player:setBumpFall(true)
  player:setBumpFallType("pushedBehind")
  sendClientCommand('BirgetModFall', 'fall', { username = player:getUsername() })
end

function isSlipTile(x,y,z,tileName)
  ...
  if sq ~= nil then
    for i=1, sq:getObjects():size() do
		...
		if obj:getSprite() ~= nil and spearSprites[obj:getTextureName()] then
          return true
        end
    end
  end
...
local function OnPlayerMove(player)
  ...
  if isSlipTile(x,y,z,tileName) then
...
local function giveSomePain(player)
...
birgetModFall_onServerCommand = function(module, command, args)
    if module == "BirgetModFall" and command == "fall" then
        local player = getPlayerFromUsername(args.username)
        player:clearVariable("BumpFallType")
        player:setBumpType("stagger")
        player:setBumpDone(false)
        player:setBumpFall(true)
...
Events.OnServerCommand.Add(birgetModFall_onServerCommand)
