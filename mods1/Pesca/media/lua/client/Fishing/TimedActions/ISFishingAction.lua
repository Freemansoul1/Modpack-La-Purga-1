...
--            getSoundManager():PlayWorldSound("getFish", false, self.character:getSquare(), 1, 20, 1, false)
		...
		if instanceof(fishItem, "Clothing") then
			...
			local chrdata = self.character:getModData();
			if fishItem:getType() == "Shoes_Random" then
				if not chrdata.shnote and ZombRand(10) == 4 then
					local shdata = fishItem:getModData();
					if not shdata.secret then
						shdata.secret = true;
						chrdata.shnote = "1";
					end
				end
...
--    local fish = Fishing.fishes[fishType.fishType];
    ...
    if fishType.size == "Small" then
		...
		if fish.little.minScale and fish.little.maxScale then
			--baseScale = fish.little.scale;
			minScale = fish.little.minScale;
			maxScale = fish.little.maxScale;
			customscale = true;
		end
    elseif fishType.size == "Medium" then
		...
		if fish.medium.minScale and fish.medium.maxScale then
			--baseScale = fish.medium.scale;
			minScale = fish.medium.minScale;
			maxScale = fish.medium.maxScale;
			customscale = true;
		else
			baseScale = 1.2;
		end
    ...
    elseif  fishType.size == "Big" then
		...
		if fish.big.minScale and fish.big.maxScale then
			--baseScale = fish.big.scale;
			minScale = fish.big.minScale;
			maxScale = fish.big.maxScale;
			customscale = true;
		else
			baseScale = 1.4;
		end
		...
		if fish.trophy.minScale and fish.trophy.maxScale then
			--baseScale = fish.trophy.scale;
			minScale = fish.trophy.minScale;
			maxScale = fish.trophy.maxScale;
			customscale = true;
		else
			baseScale = 1.6;
		end
	...
	if not customscale then
		...
		local Mult = (size - minSize) * ((maxScale - minScale) / (maxSize - minSize));
		local NewScale = minScale + tonumber(string.format("%." .. (4 or 0) .. "f", (Mult)));
		NewScale = tonumber(string.format("%." .. (4 or 0) .. "f", (NewScale)))
		if isDebugEnabled() then
			print("Fishing Custom Scale Manager")
			print("Fish: " .. tostring(fish.name))
			print("Size: " .. tostring(fishType.size))
			print("Scale: (" .. tostring(minScale) .. ") + Mult: (" .. tostring(tonumber(string.format("%." .. (4 or 0) .. "f", (Mult)))) .. ")");
			print("NewScale: " .. tostring(NewScale))
		end
		--self.character:Say("Mult " .. tostring(tonumber(string.format("%." .. (4 or 0) .. "f", (Mult)))) .. ", " .. "NewScale " .. tostring(NewScale));

		fishToCreate:setWorldScale(NewScale);
	end
