...
function OpenGate(square)
	...
	getSoundManager():PlayWorldSound("gateopen", false, square, 0.2, 60, 0.2, false) ;								
...
function CloseGate(square)
	...
	getSoundManager():PlayWorldSound("gateclose", false, square, 0.2, 60, 0.2, false) ;
end
...
function GateOnPlayerUpdate(player)
	...
	if(Ticks % 40 == 0) then
		...
		player:Say("isGate:".. tostring(result) .. "  " .. "GatePos:".. tostring(result2));
	end
end
